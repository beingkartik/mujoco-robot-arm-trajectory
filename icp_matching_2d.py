...
# -*- coding: utf-8 -*-
"""
Created on Sun Mar 22 14:39:10 2020
...
# -*- coding: utf-8 -*-
"""
Created on Sun Mar 22 14:15:32 2020
...
class icp_matching_2d():
    
    def __init__(self):
      pass
  
    def icp(self,data,template,Tr,max_time=1):
        t0 = time.time()
        init_pose = (0,0,0) #dx,dy,dtheta(in radian)
        src = numpy.array([data], copy=True).astype(numpy.float32)
        dst = numpy.array([template], copy=True).astype(numpy.float32)
...
#                       [0,                    0,                   1          ]])
        ...
        while not(first and time.time() - t0 > max_time):
            ...
            p = scipy.optimize.minimize(self.res,[0,0,0],args=(src[0],dst[0, indices.T][0]),method='Newton-CG',jac=self.jac,hess=self.hess).x
            ...
            error = self.res([0,0,0],src[0],dst[0, indices.T][0])
